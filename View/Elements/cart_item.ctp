...
echo $this->Form->input("TransactionItem.{$i}.quantity", array(
    ...
    'size' => 1,
    'after' => __(' %s', $this->Html->link('<i class="glyphicon glyphicon-trash"></i>', array('plugin' => 'transactions', 'controller' => 'transaction_items', 'action' => 'delete', $transactionItem['id']), array('title' => 'Remove from cart', 'escape' => false)))
    ));
